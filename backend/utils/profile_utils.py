...
def update_profile_info(user_id, username, avatar_url, db_conn):
    ...
    if avatar_url:
        ...
        cursor.execute(
            'UPDATE Users SET username = %s, profile_image_path = %s '
            'WHERE user_id = %s',
            (username, full_path, user_id)
        )
    else:
        cursor.execute(
...
def fetch_profile_info(user_id, db_conn):
    cursor = db_conn.cursor(dictionary=True)
    cursor.execute(
        'SELECT username, profile_image_path, role FROM Users '
        'WHERE user_id = %s',
        (user_id,)
    )
    user = cursor.fetchone()
    ...
    if user:
        # Always return the secure route for avatar_url
        avatar_url = (
            f"/api/profile/avatar/{user_id}"
            if user.get('profile_image_path') else None
        )
        return {
            'username': user['username'],
...
def get_profile_avatar_path(user_id, db_conn=None):
    ...
    cursor = db_conn.cursor(dictionary=True)
    cursor.execute(
        'SELECT profile_image_path FROM Users WHERE user_id = %s',
        (user_id,)
    )
    user = cursor.fetchone()
    cursor.close()
...
def save_profile_avatar(file_or_bytes, user_id, user_role, is_raw_bytes=False):
    """
    Save uploaded avatar file to disk in the correct folder based on user role
    and return the full OS path. Returns (avatar_path, error) tuple.
    """
    import os
    from datetime import datetime
    try:
        filename = (
            f'user_{user_id}_avatar_'
        ...
        if user_role == 'admin':
            avatar_rel_path = os.path.join(
                'resources', 'profile', 'admin', filename
            )
        else:
            avatar_rel_path = os.path.join(
                'resources', 'profile', 'resident', filename
            )
        ...
        if is_raw_bytes:
            ...
            file_or_bytes.save(avatar_path)
